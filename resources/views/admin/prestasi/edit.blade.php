            ...
            <div class="grid gap-5">
                ...
                <div>
                    ...
                    <select name="ekstrakurikuler_id" class="form-input">
                        ...
                        @foreach($ekskul as $e)
                        <option value="{{ $e->id }}"
                                {{ old('ekstrakurikuler_id', $prestasi->ekstrakurikuler_id) == $e->id ? 'selected' : '' }}>
                            {{ $e->nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="tahun" class="form-input w-36 @error('tahun') border-red-400 @enderror"
                           value="{{ old('tahun', $prestasi->tahun) }}" min="2000" max="{{ date('Y') + 1 }}" required>
                    @error('tahun')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4"
                              class="form-input @error('deskripsi') border-red-400 @enderror">{{ old('deskripsi', $prestasi->deskripsi) }}</textarea>
                    @error('deskripsi')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <img src="{{ Storage::url($prestasi->foto) }}"
                         class="h-24 rounded-xl object-cover mb-2" alt="{{ $prestasi->judul }}">
                    @endif
                    <input type="file" name="foto" class="form-input @error('foto') border-red-400 @enderror"
                           accept="image/*">
                    <p class="text-xs text-gray-400 mt-1">Format gambar. Maks 2MB. Kosongkan jika tidak ingin mengganti.</p>
                    @error('foto')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
