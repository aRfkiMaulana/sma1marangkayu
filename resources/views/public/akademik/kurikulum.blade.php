...
@php
    ...
    $subjudul = $meta['subjudul'] ?? 'SMA Negeri 1 Marangkayu menerapkan Kurikulum Merdeka sebagai pedoman penyelenggaraan pembelajaran yang berpusat pada siswa, fleksibel, dan relevan dengan kebutuhan zaman.';
        ...
        <div class="bg-white rounded-3xl border border-gray-200 overflow-hidden mb-6">
            <div class="relative overflow-hidden px-6 py-8 md:px-10 md:py-14 text-white">
                ...
                <div class="relative max-w-2xl">
                    ...
                    <h1 class="text-3xl md:text-4xl font-bold text-white leading-tight mb-3">{{ $kurikulum->judul ?? 'Kurikulum Merdeka' }}</h1>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="relative overflow-hidden bg-white rounded-2xl p-6 border border-gray-200">
                    ...
                    <div class="relative z-10">
                        <h2 class="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                            ...
                            Apa itu {{ $kurikulum->judul ?? 'Kurikulum Merdeka' }}?
                        </h2>
                        <div class="space-y-3 text-sm text-gray-600 leading-relaxed">
                            @if($kurikulum && $kurikulum->konten)
                                {!! nl2br(e($kurikulum->konten)) !!}
                            @else
                                ...
                                <p>Di SMAN 1 Marangkayu, Kurikulum Merdeka diterapkan secara penuh dengan pendekatan pembelajaran berbasis proyek (<strong class="text-gray-900">Project-Based Learning</strong>) dan penguatan profil pelajar Pancasila.</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-200">
                    ...
                    <div class="space-y-3">
                        @php
                            $colors = [
                                ['bg'=>'bg-blue-50','border'=>'border-blue-100','icon_color'=>'text-blue-600','icon'=>'fa-user-graduate'],
                                ['bg'=>'bg-emerald-50','border'=>'border-emerald-100','icon_color'=>'text-emerald-600','icon'=>'fa-seedling'],
                                ['bg'=>'bg-amber-50','border'=>'border-amber-100','icon_color'=>'text-amber-600','icon'=>'fa-diagram-project'],
                                ['bg'=>'bg-purple-50','border'=>'border-purple-100','icon_color'=>'text-purple-600','icon'=>'fa-sliders'],
                                ['bg'=>'bg-sky-50','border'=>'border-sky-100','icon_color'=>'text-sky-600','icon'=>'fa-chart-line'],
                            ];
                        @endphp
                        @foreach($tujuanList as $i => $t)
                        @php $c = $colors[$i % count($colors)]; @endphp
                        <div class="flex items-start gap-4 p-4 rounded-xl border {{ $t['bg'] ?? $c['bg'] }} {{ $t['border'] ?? $c['border'] }}">
                            <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-white">
                                <i class="fa-solid {{ $t['icon'] ?? $c['icon'] }} {{ $t['icon_color'] ?? $c['icon_color'] }}"></i>
                            </div>
                            <div>
            ...
            <div class="space-y-5 lg:sticky lg:top-40 self-start">
                ...
                <div class="bg-white rounded-2xl p-5 border border-gray-200">
                    <h3 class="font-bold text-gray-900 text-sm mb-3 flex items-center gap-2">
                        ...
                        Dokumen Kurikulum Resmi (KOS)
                    </h3>
                    <p class="text-xs text-gray-500 mb-4">Unduh dokumen Kurikulum Operasional Sekolah resmi SMAN 1 Marangkayu.</p>
                    <a href="{{ Storage::url($kurikulum->file_lampiran) }}" download target="_blank" class="btn-primary text-xs w-full justify-center">
                        <i class="fa-solid fa-download"></i> Unduh Dokumen (PDF)
                    </a>
                </div>
                @endif
